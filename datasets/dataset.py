...
def get_dataset_filelist(a):
    with open(a.input_training_file, 'r', encoding='utf-8') as fi:
        training_indexes = [x.split('|') for x in fi.read().split('\n') if len(x) > 0]

    with open(a.input_validation_file, 'r', encoding='utf-8') as fi:
        validation_indexes = [x.split('|') for x in fi.read().split('\n') if len(x) > 0]
...
class Val_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        file_info = self.audio_indexes[index]
        if len(file_info) >= 3:
            # 新格式：包含clean和noisy路径
            filename, clean_path, noisy_path = file_info[0], file_info[1], file_info[2]
        elif len(file_info) >= 2:
            # 旧格式：包含文件名和clean路径
            filename, clean_path = file_info[0], file_info[1]
        ...
        if self._cache_ref_count == 0:
            clean_audio, _ = librosa.load(clean_path, self.sampling_rate)
            noisy_audio, _ = librosa.load(noisy_path, self.sampling_rate)
